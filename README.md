...
**去过的地方会被降权。** 每次确认「就这儿」都记一笔，下次抽取时优先给没去过的格子（2km 网格）。首页显示这一片的探索度 —— 应用越用越有价值，随机不会老在同一片打转。

**双人模式，不需要后端。** 三选一阶段点「让对方来选」，三个候选的坐标塞进分享 path，对方打开直接进选择界面。我负责随机，你负责拍板。

**「最远不超过 X 公里」。** 首页可选距离上限，力度 100% 就等于这个上限而不是区域最远点 —— 给「选上海市结果抽到崇明岛」兜底。

**雨天感知。** 配了高德 Key 就会查天气，下雨自动把周边推荐默认切到「逛街」（室内）。
...
apps/
  dart-map/                  主应用
    ...
    config.example.js        Key 模板（config.js 被 gitignore）
    utils/lbs.js             位置服务适配层 ★
    utils/visited.js         探索记录 / 探索度 / 去过降权（纯本地 Storage）
    utils/weather.js         天气适配层（高德，可选）
    assets/geo/
...
| `game.js` `_pickCandidates` | 超采样 16 个成池 → 没去过的排前面 → 贪心取 3 个并保证间距。**间距约束放在最终选择上而不是池子上**，否则池子撑不大、可选空间太小 |
| `visited.js` `cellIdOf` | 2km 网格。经度方向按纬度做 cos 修正，高纬度格子才不会被压扁 |
...
| 优先级 | 功能 |
|---|---|
| P1 | 历史记录页（数据已经在存了，缺一个列表界面 + 地图上点亮去过的地方） |
| P1 | 结果分享生成带标记的图片（文字分享已做） |
| P2 | 接高德拿评分（方案已写好） |
| P2 | 用通勤时间代替直线距离（现在的「最远 X 公里」是直线距离近似，真实可达性要路径规划 API） |
